#ifndef GCE_Volume_Move_For_NPT_H_
#define GCE_Volume_Move_For_NPT_H_
...
void GCE_Volume_Move_For_NPT_Function (int *steps, Particle_Model *p_head, dsfmt_t *dsfmt, double *dr, double *accept, double *reject, double *bx, double *by, double *bz)
{
	if (*steps > INT_MAX)
    {
    ...
    double v_old, lnvn, v_new, boxn;
    v_old = (*bx) * (*by) * (*bz);
    lnvn = log (v_old) + (dsfmt_genrand_open_open (dsfmt) - 0.5) * V_max;
    v_new = exp (lnvn);
    boxn = pow (v_new, 1.0/3.0);
    ...
    else
    {
        ...
        acc = exp (-1.0 * delta_enthalpy * BETA_RT - 0.5 * Alpha * delta_enthalpy * (delta_enthalpy + 2.0 * (former_energy + P_Reference * v_old ) - 2.0 * H0) + (Particle_Number + 1) * log (v_new / v_old));
        /** End **/
...
void GCE_Volume_Move_For_NPT_Function_Mpi (int id, int *steps, Particle_Model *p_head, dsfmt_t *dsfmt, double *dr, double *accept, double *reject, double *bx, double *by, double *bz)
{
    if (*steps > INT_MAX)
    {
        printf ("ERROR! Step exceeds INT_MAX!\n");
